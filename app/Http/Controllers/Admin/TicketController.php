...
class TicketController extends AdminBaseController
{
    ...
    {
         abort_403(!$this->user->roles()->withoutGlobalScopes()->first()->hasPermission(['read_ticket','create_ticket', 'update_ticket', 'delete_ticket']));
        ...
        if (\request()->ajax()) {
            ...
            return \datatables()->of($ticket)
                ->addColumn('action', function ($row) {
                    ...
                    if ($this->user->roles()->withoutGlobalScopes()->first()->hasPermission('delete_ticket') && ($row->status != 'closed')) {
    ...
    {
        abort_403(!$this->user->roles()->withoutGlobalScopes()->first()->hasPermission('create_ticket'));
    ...
    {
        abort_403(!$this->user->roles()->withoutGlobalScopes()->first()->hasPermission('update_ticket'));
