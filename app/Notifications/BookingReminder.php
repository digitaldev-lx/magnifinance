...
class BookingReminder extends BaseNotification implements ShouldQueue
{
 use Queueable;
    /**
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ...
            ->line(__('email.bookingReminder.text') . $this->booking->company->company_name)
            ->line(__('app.booking').' #'.$this->booking->id)
            ->line(__('app.at').' - '.$this->booking->date_time->format($this->booking->company->date_format.' '.$this->booking->company->time_format))
    ...
    public function toNexmo($notifiable)
    {
        return (new NexmoMessage)
            ->content(
                        __('email.bookingReminder.text').$this->booking->company->company_name."\n".
                        __('app.booking').' #'.$this->booking->id."\n".
                        __('app.at').' - '.$this->booking->date_time->format($this->booking->company->date_format.' '.$this->booking->company->time_format))
            ->unicode();
    }
