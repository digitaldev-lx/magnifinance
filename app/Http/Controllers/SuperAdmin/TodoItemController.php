        ...
        if (request()->ajax()) {
            return datatables()->of($this->user->todoItems)
                ->addColumn('action', function ($row) {
                    $action = '<div class="text-right"><a href="javascript:showUpdateFeedbackForm(' . $row->id . ');" class="btn btn-primary btn-circle edit-todo" data-toggle="tooltip" data-original-title="'.__('app.edit').'"><i class="fa fa-pencil" aria-hidden="true"></i></a>
    ...
    public function updateTodoItem(Request $request)
    {
        ...
        if ($request->exists('status')) {
            if ($request->status == 'completed') {
                ...
                $pendingTodoItemsGreaterThanOldPosition = $this->todoItems->filter(function ($value, $key) use($newPosition) {
